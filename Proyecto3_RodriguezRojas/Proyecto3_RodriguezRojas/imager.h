...
namespace Imager
{
	...
	class backGround : public SolidObject_Reorientable
	{
	public:
		backGround(unsigned char* backGroundColors) : SolidObject_Reorientable()
		{
			SetTag("BackGround");   // tag for debugging
			int c = 0;
			//15718563
			for (int i = 0; i < 1008; i++) {
				for (int j = 0; j < 567; j++) {
					Container[i][j] = Color(backGroundColors[c], backGroundColors[c+1], backGroundColors[c+2]);
				}
				c += 3;
			}
		}
